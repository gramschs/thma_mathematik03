...
Hunderttausenden von Unbekannten, die oft für mehrere Lastfälle gelöst werden
müssen. *Können wir die Inverse nutzen, um solche Gleichungssysteme zu lösen?*
In diesem Kapitel schreiben wir lineare Gleichungssysteme in Matrixform und
lösen sie mit der Inversen, sogar für mehrere rechte Seiten auf einmal.
...
Umgekehrt können wir aus der Matrixschreibweise jederzeit das Gleichungssystem
zurückgewinnen. Mit der Matrix $\mathbf{B}$ steht zum Beispiel
...
Hier können wir die Lösung $x_2 = 3$ und $x_1 = 5$ sofort ablesen, weil die
zweite Gleichung nur eine Unbekannte enthält. Im Allgemeinen ist das nicht so
einfach.
...
(2, 0)^{\top}$ müssen wir nichts neu umformen, sondern nur noch einmal
multiplizieren:
...
Matrizen brauchen wir den Gauß-Jordan-Algorithmus. Müssen wir ein
...
$\mathbf{A}$ nacheinander gelöst. Nun nehmen wir eine dritte rechte Seite
$\vec{b}_3 = (0, 2)^{\top}$ hinzu. *Müssen wir jedes Gleichungssystem einzeln
...
Gleichung $\mathbf{X} \cdot \mathbf{A} = \mathbf{C}$ mit $\mathbf{C} = \begin{pmatrix} 0 & 2 \\ 2 & 0 \end{pmatrix}$. Jetzt müssen wir von rechts mit
$\mathbf{A}^{-1}$ multiplizieren, damit $\mathbf{A}$ und $\mathbf{A}^{-1}$
nebeneinanderstehen. Rechts von $\mathbf{X}$ bleibt dann $\mathbf{A} \cdot
\mathbf{A}^{-1} = \mathbf{E}$ stehen.
...
Die Probe $\mathbf{X} \cdot \mathbf{A} = \begin{pmatrix} 1 & 3 \\ 1 & 1
\end{pmatrix} \cdot \begin{pmatrix} 3 & -1 \\ -1 & 1 \end{pmatrix} =
\begin{pmatrix} 0 & 2 \\ 2 & 0 \end{pmatrix}$ bestätigt das Ergebnis. Hätten
wir versehentlich von links multipliziert, hätten wir $\mathbf{A}^{-1} \cdot
\mathbf{C} = \begin{pmatrix} 1 & 1 \\ 3 & 1 \end{pmatrix}$ erhalten. Die Probe
wäre dann gescheitert, denn $\begin{pmatrix} 1 & 1 \\ 3 & 1 \end{pmatrix} \cdot
\mathbf{A} = \begin{pmatrix} 2 & 0 \\ 8 & -2 \end{pmatrix} \neq \mathbf{C}$.
