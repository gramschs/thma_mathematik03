...
kennengelernt: Streckungen, Spiegelungen, Projektionen und Scherungen. All
diese Transformationen haben gemeinsam, dass sie durch eine Matrix-Vektor-Multi\-plikation
beschrieben werden. In diesem Kapitel definieren wir den Begriff der linearen
Abbildung formal und lernen die zwei Eigenschaften kennen, die jede lineare Abbildung
erfüllt.
...
* Sie können überprüfen, ob eine gegebene Abbildung linear ist, indem Sie Homogenität
  und Additivität nachweisen.
* Sie wissen, dass nicht jede Abbildung, die geometrisch einfach wirkt,
  auch linear ist, und können Gegenbeispiele angeben.
```
...
gestreckt oder gestaucht, so ist das Ergebnis dasselbe, als würde man den
Originalvektor abbilden und anschließend strecken oder stauchen.
...
Dies lässt sich auch direkt einsehen: Die Multiplikation einer Matrix mit dem
Nullvektor ergibt immer den Nullvektor. Diese Eigenschaft ist ein nützliches
Kriterium: Eine Abbildung, die den Nullvektor nicht auf den Nullvektor abbildet,
kann nicht linear sein.
...
Summe wie die Abbildung der einzelnen Vektoren.
...
die in der Technik als **Superpositionsprinzip** bekannt ist:
...
Vektoren gleich der entsprechenden Linearkombination der Bildvektoren ist. Dies ist
eine grundlegende Eigenschaft vieler physikalischer Systeme: Bei linearen Systemen
(lineare Federsteifigkeit, ohmscher Widerstand, usw.) können Teilwirkungen einzeln
berechnet und dann überlagert werden.
...
Ein weiteres Beispiel: Die Abbildung $f(x) = x^2$ in einer Dimension ist ebenfalls
nicht linear, weil etwa $f(2) = 4 \neq 2 \cdot f(1) = 2$.

```{admonition} Lineare Abbildung: Homogenität und Additivität
:class: note
Eine Abbildung $F_{\mathbf{A}}: \mathbb{R}^n \to \mathbb{R}^m$ mit
$F_{\mathbf{A}}(\vec{v}) = \mathbf{A} \cdot \vec{v}$ ist linear, weil sie
...
\mathbb{R}^n$ und alle $\alpha \in \mathbb{R}$.
...
Das folgende Video erklärt die Eigenschaften linearer Abbildungen anschaulich.
...
Superpositionsprinzip wider. Nicht jede Abbildung ist linear: Translationen und
nichtlineare Funktionen verletzen diese Eigenschaften. In den nächsten Kapiteln
werden wir zwei wichtige Konzepte kennenlernen, die beschreiben, welche Vektoren
durch eine lineare Abbildung auf den Nullvektor abgebildet werden (Kern) und welche
Vektoren als Ergebnis einer Abbildung auftreten können (Bild).
