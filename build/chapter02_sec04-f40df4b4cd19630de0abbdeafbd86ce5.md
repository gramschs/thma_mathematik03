...
Im letzten Abschnitt haben wir gesehen, dass sich die Inverse lohnt, wenn wir
dasselbe Gleichungssystem für mehrere rechte Seiten lösen wollen. In der
Technischen Mechanik ist das zum Beispiel der Fall, wenn ein Bauteil für
mehrere Lastfälle berechnet wird. Die Koeffizientenmatrix bleibt dann gleich,
nur die rechte Seite ändert sich. *Müssen wir dafür wirklich jedes
Gleichungssystem einzeln lösen?*

Wir nehmen die Matrix $\mathbf{A}$ aus Kapitel 2.3, deren Inverse wir bereits
kennen:
...
Wir wollen die drei Gleichungssysteme $\mathbf{A} \cdot \vec{x} = \vec{b}$ mit
den rechten Seiten $\vec{b}_1 = (0, 1)^{\top}$, $\vec{b}_2 = (2, 0)^{\top}$ und
$\vec{b}_3 = (2, -1)^{\top}$ lösen. Statt drei einzelne Gleichungssysteme
aufzuschreiben, stellen wir die rechten Seiten als Spalten nebeneinander:

\begin{equation*}
\mathbf{B} = \begin{pmatrix} 0 & 2 & 2 \\ 1 & 0 & -1 \end{pmatrix}.
\end{equation*}
...
eine ganze Matrix. Um sie zu bestimmen, multiplizieren wir beide Seiten von
links mit $\mathbf{A}^{-1}$, genau wie beim einzelnen Gleichungssystem. Links
bleibt $\mathbf{A}^{-1} \cdot \mathbf{A} \cdot \mathbf{X} = \mathbf{E} \cdot
\mathbf{X} = \mathbf{X}$ stehen.
...
Für unser Beispiel erhalten wir alle drei Lösungen mit einer einzigen
Matrizenmultiplikation:

\begin{equation*}
\mathbf{X} = \mathbf{A}^{-1} \cdot \mathbf{B} = \frac{1}{2} \begin{pmatrix} 1 &
1 \\ 1 & 3 \end{pmatrix} \cdot \begin{pmatrix} 0 & 2 & 2 \\ 1 & 0 & -1
\end{pmatrix} = \frac{1}{2} \begin{pmatrix} 1 & 2 & 1 \\ 3 & 2 & -1
\end{pmatrix}.
\end{equation*}

Die erste Spalte ist die Lösung des ersten Gleichungssystems, die zweite Spalte
die des zweiten und die dritte Spalte die des dritten. Die Probe bestätigt alle
drei Lösungen auf einmal:

\begin{equation*}
\mathbf{A} \cdot \mathbf{X} = \begin{pmatrix} 3 & -1 \\ -1 & 1 \end{pmatrix}
\cdot \frac{1}{2} \begin{pmatrix} 1 & 2 & 1 \\ 3 & 2 & -1 \end{pmatrix} =
\frac{1}{2} \begin{pmatrix} 0 & 4 & 4 \\ 2 & 0 & -2 \end{pmatrix} =
\mathbf{B}.
\end{equation*}

*Und wenn die Unbekannte links steht?* Betrachten wir die Gleichung
$\mathbf{X} \cdot \mathbf{A} = \mathbf{C}$ mit
...
Jetzt steht $\mathbf{A}$ rechts von der Unbekannten, also multiplizieren wir von
rechts mit $\mathbf{A}^{-1}$ und erhalten
...
\begin{equation*}
\mathbf{X} \cdot \mathbf{A} = \begin{pmatrix} 1 & 3 \\ 1 & 1 \end{pmatrix} \cdot \begin{pmatrix} 3 & -1 \\ -1 & 1 \end{pmatrix} =
\begin{pmatrix} 0 & 2 \\ 2 & 0 \end{pmatrix}
\end{equation*}

bestätigt das Ergebnis. Hätten wir
versehentlich von links multipliziert, hätten wir

\begin{equation*}
\mathbf{A}^{-1} \cdot \mathbf{C} =
\begin{pmatrix} 1 & 1 \\ 3 & 1 \end{pmatrix}
\end{equation*}

erhalten, und die Probe wäre gescheitert.
...
Im ersten Beispiel ist das falsche Produkt $\mathbf{B} \cdot \mathbf{A}^{-1}$
nicht einmal definiert, denn eine $2\times 3$-Matrix lässt sich nicht mit einer
$2\times 2$-Matrix multiplizieren.
```
...
sogar mehrere Gleichungssysteme in einem einzigen Schritt lösen. Insbesondere Computerprogramme und
