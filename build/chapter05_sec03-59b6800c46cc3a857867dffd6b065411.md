...
In Abschnitt 5.2 haben wir gesehen, dass symmetrische Matrizen immer
diagonalisierbar sind. Aber was bedeutet das eigentlich genau? Die
Diagonalisierung ist ein Wechsel des Koordinatensystems: Dieselbe physikalische
Transformation, etwa die Biegesteifigkeit eines Querschnitts, sieht im
Koordinatensystem der Hauptträgheitsachsen diagonal aus, im ursprünglichen
Koordinatensystem aber nicht. Mathematisch steckt dahinter der Begriff der
ähnlichen Matrizen, den wir in diesem Abschnitt einführen.
...
* [ ] Sie wissen, dass ähnliche Matrizen dasselbe charakteristische Polynom
  und damit dieselben Eigenwerte besitzen.
* [ ] Sie können nachrechnen, ob zwei gegebene Matrizen ähnlich sind.
```
...
direkt nachrechnen. Wir berechnen das charakteristische Polynom von
$\mathbf{C} = \mathbf{B}^{-1}\mathbf{A}\mathbf{B}$:

\begin{align*}
\det(\mathbf{C} - \lambda\mathbf{E})
&= \det(\mathbf{B}^{-1}\mathbf{A}\mathbf{B} - \lambda\mathbf{B}^{-1}\mathbf{E}\mathbf{B}) \\
&= \det(\mathbf{B}^{-1}(\mathbf{A} - \lambda\mathbf{E})\mathbf{B}) \\
&= \det(\mathbf{B}^{-1})\cdot\det(\mathbf{A} - \lambda\mathbf{E})\cdot\det(\mathbf{B}).
\end{align*}

Da $\det(\mathbf{B}^{-1}) = 1/\det(\mathbf{B})$ gilt, heben sich die beiden
Determinanten von $\mathbf{B}$ und $\mathbf{B}^{-1}$ auf. Wir erhalten:

\begin{equation*}
\det(\mathbf{C} - \lambda\mathbf{E}) = \det(\mathbf{A} - \lambda\mathbf{E}).
\end{equation*}

Ähnliche Matrizen haben also dasselbe charakteristische Polynom und damit
dieselben Eigenwerte, einschließlich ihrer algebraischen Vielfachheiten.

```{admonition} Ähnliche Matrizen haben dieselben Eigenwerte
:class: note
Sind $\mathbf{A}$ und $\mathbf{C} = \mathbf{B}^{-1}\mathbf{A}\mathbf{B}$
ähnliche Matrizen, dann gilt:

\begin{equation*}
\det(\mathbf{C} - \lambda\mathbf{E}) = \det(\mathbf{A} - \lambda\mathbf{E}).
\end{equation*}

Insbesondere haben $\mathbf{A}$ und $\mathbf{C}$ dieselben Eigenwerte mit
denselben algebraischen Vielfachheiten.
```

Wir überprüfen das an unserem Beispiel. Die charakteristischen Polynome von
$\mathbf{I}$ und $\mathbf{D}$ sind:
...
stimmen überein. Das Ergebnis ist konsistent: Die Hauptträgheitsmomente hängen
nicht davon ab, in welchem Koordinatensystem wir den Tensor aufschreiben.
