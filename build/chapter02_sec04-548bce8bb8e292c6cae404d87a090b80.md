...
mit den beiden Unbekannten $x_1$ und $x_2$. Die linken Seiten erinnern an die
Matrizenmultiplikation aus Kapitel 1.4. Fassen wir die Zahlen vor den
...
In unserem Beispiel ist also $\mathbf{A} = \begin{pmatrix} 3 & -1 \\ -1 & 1
\end{pmatrix}$ die Koeffizientenmatrix und $\vec{b} = (1, 1)^{\top}$ die rechte
Seite. Umgekehrt können wir aus der Matrixschreibweise jederzeit das
Gleichungssystem zurückgewinnen. Mit der Matrix $\mathbf{B}$ aus Kapitel 2.3
steht zum Beispiel
...
für die beiden Gleichungen $x_1 - x_2 = 2$ und $x_2 = 3$. Hier können wir die
Lösung $x_2 = 3$ und $x_1 = 5$ sofort ablesen, weil die zweite Gleichung nur
eine Unbekannte enthält. Im Allgemeinen ist das nicht so einfach.
...
links mit $\mathbf{A}^{-1}$. Links bleibt $\mathbf{A}^{-1} \cdot \mathbf{A}
\cdot \vec{x} = \mathbf{E} \cdot \vec{x} = \vec{x}$ stehen, und wir erhalten
$\vec{x} = \mathbf{A}^{-1} \cdot \vec{b}$. Wichtig ist, dass wir von links
multiplizieren, denn nur dort steht $\mathbf{A}$ neben $\mathbf{A}^{-1}$.
...
Gleichung $\mathbf{X} \cdot \mathbf{A} = \mathbf{C}$ mit $\mathbf{C} =
\begin{pmatrix} 0 & 2 \\ 2 & 0 \end{pmatrix}$. Jetzt müssen wir von rechts mit
