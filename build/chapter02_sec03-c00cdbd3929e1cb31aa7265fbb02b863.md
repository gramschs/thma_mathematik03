...
Wir betrachten zwei Massen, die über zwei Federn hintereinander an einer festen
Wand befestigt sind. Ziehen wir an den Massen, verschieben sie sich so weit, bis
die Federkräfte die äußeren Kräfte ausgleichen. Für die Federsteifigkeiten $k_1
= 2~\text{kN/mm}$ und $k_2 = 1~\text{kN/mm}$ hängen die Kräfte $F_1, F_2$ an
den Massen und deren Verschiebungen $u_1, u_2$ über $\vec{F} = \mathbf{K} \cdot
\vec{u}$ zusammen, mit
...
Wie diese Matrix aus den Federsteifigkeiten entsteht, lernen Sie in der
Technischen Mechanik. Für uns genügt es zu wissen, was sie leistet. Die Matrix
$\mathbf{K}$ heißt **Steifigkeitsmatrix** und beantwortet die Frage, welche
Kräfte wir brauchen, um bestimmte Verschiebungen zu erzeugen. Im Versuch ist die
Frage aber meist umgekehrt. *Wie weit verschieben sich die Massen, wenn wir an
Masse 2 mit $1~\text{kN}$ ziehen?*

Dazu brauchen wir die Inverse. Mit der Formel aus dem letzten Kapitel und $a
\cdot d - c \cdot b = 3 \cdot 1 - (-1) \cdot (-1) = 2$ erhalten wir die
...
\vec{u} = \mathbf{K}^{-1} \cdot \begin{pmatrix} 0 \\ 1 \end{pmatrix}~\text{kN} =
\begin{pmatrix} 0{,}5 \\ 1{,}5 \end{pmatrix}~\text{mm}.
\end{equation*}

Masse 2 bewegt sich weiter als Masse 1, weil zwischen ihr und der Wand beide
Federn nachgeben. Die Probe $\mathbf{K} \cdot \vec{u} = (3 \cdot 0{,}5 - 1 \cdot
1{,}5; \; -1 \cdot 0{,}5 + 1 \cdot 1{,}5)^{\top} = (0; 1)^{\top}~\text{kN}$
liefert genau die Kraft zurück, mit der wir gestartet sind.
...
und schon bei Zahlen ist $1/(2+3) \neq 1/2 + 1/3$. Bauen wir eine zweite,
baugleiche Federkette daneben ein, tragen doppelt so viele Federn die Last. Die
Steifigkeitsmatrix wird zu $\mathbf{K} + \mathbf{K} = 2 \cdot \mathbf{K}$, und
nach der Skalarregel halbieren sich die Verschiebungen. Würden wir stattdessen
die Nachgiebigkeiten addieren, ergäbe sich $\mathbf{K}^{-1} + \mathbf{K}^{-1} =
2 \cdot \mathbf{K}^{-1}$, also die doppelte Verschiebung, obwohl das System
...
Zylinder 1 bringt die Kraft $s_1$ auf Masse 1 auf. Zylinder 2 drückt mit $s_2$
auf Masse 2 und stützt sich dabei an Masse 1 ab, drückt diese also mit $s_2$
zurück. Für die Kräfte an den Massen gilt also $F_1 = s_1 - s_2$ und $F_2 =
s_2$, in Matrixschreibweise $\vec{F} = \mathbf{H} \cdot \vec{s}$ mit
...
Masse 1 stehen, und nur Masse 2 bewegt sich um $1~\text{mm}$, weil Zylinder 2
lediglich Feder 2 dehnt. Für den Versuch interessiert uns aber wieder die
umgekehrte Frage. *Welche Stellkräfte
...
Für die Zylinder brauchen wir noch $\mathbf{H}^{-1}$. Wegen $1 \cdot 1 - 0 \cdot
...
1)^{\top}~\text{kN}$. Das ist physikalisch einleuchtend. Zylinder 1 hält Feder 1
um $1~\text{mm}$ gedehnt und braucht dafür $2~\text{kN}$, Zylinder 2 hält Feder 2
...
Hydraulikzylinder haben einen begrenzten Hub. *Wie weit muss jeder Kolben
ausfahren, wenn sich die Massen um $u_1$ und $u_2$ verschieben?* Der Kolben von
Zylinder 1 fährt genau um $u_1$ aus. Zylinder 2 sitzt zwischen den Massen, sein
Kolben fährt deshalb um die Differenz $u_2 - u_1$ aus. Für die Kolbenwege $d_1$
und $d_2$ gilt also
...
Die Matrix, die Verschiebungen in Kolbenwege übersetzt, ist genau die
Transponierte der Matrix $\mathbf{H}$, die Stellkräfte in Kräfte übersetzt. Das
ist kein Zufall, den Grund dafür lernen Sie in der Technischen Mechanik kennen.
Auch hier interessiert uns die umgekehrte Frage. *Welche Verschiebungen ergeben
sich, wenn wir die Kolbenwege vorgeben?*
...
\cdot \mathbf{A})^{\top} = \mathbf{E}^{\top} = \mathbf{E}$. Für die Zylinder
bedeutet das: Fahren beide Kolben um $\vec{d} = (1, 1)^{\top}~\text{mm}$ aus,
...
$(\mathbf{K}^{-1})^{\top} = (\mathbf{K}^{\top})^{-1} = \mathbf{K}^{-1}$. In der
Technischen Mechanik werden Sie diese Beobachtung als Satz von Maxwell-Betti
wiedertreffen.
