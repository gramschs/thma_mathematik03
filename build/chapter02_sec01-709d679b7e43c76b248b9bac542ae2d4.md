...
Nachgerechnet an unserem Beispiel ist $\mathbf{A}+\mathbf{B} =
\begin{pmatrix} 3 & 3 \\ 1 & 3 \end{pmatrix}$, also $\left(\mathbf{A}+\mathbf{B}\right)^{\top} =
\begin{pmatrix} 3 & 1 \\ 3 & 3 \end{pmatrix}$. Andererseits ist $\mathbf{A}^{\top} =
