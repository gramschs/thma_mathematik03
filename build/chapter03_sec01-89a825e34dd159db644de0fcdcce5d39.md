...
Matrizen sind nicht nur Tabellen von Zahlen – sie beschreiben geometrische
Transformationen. In diesem Kapitel lernen wir, wie eine Matrix einen Vektor in
einen neuen Vektor umwandelt, und warum das für den Maschinenbau unverzichtbar
ist.
...
durch die Abbildung zu schicken. Denn jeder beliebige Vektor
...
wird um den gleichen Faktor $s > 0$ von der Mitte weg verschoben – das Profil
wird vergrößert. Die zugehörige Abbildungsmatrix ist
...
Eine weitere wichtige Transformation ist die **Spiegelung**. Im
Koordinatensystem einer Maschine kann es notwendig sein, ein Profil an einer
Symmetrieachse zu spiegeln, um die gespiegelte Variante eines Bauteils zu
erzeugen.
...
Die Basisvektoren werden also vertauscht. Für einen beliebigen Vektor gilt:
...
Der Punkt $(x, y)$ wird auf den Punkt $(y, x)$ abgebildet – genau die
...
Der Betrag $|\det| = 1$ zeigt, dass der Flächeninhalt erhalten bleibt. Das
negative Vorzeichen zeigt an, dass die Orientierung umgekehrt wird – ein
ursprünglich gegen den Uhrzeigersinn beschriebenes Profil wird nach der Abbildung
im Uhrzeigersinn beschrieben.

## Scherung

Eine **Scherung** (engl. *shear*) verschiebt Punkte parallel zu einer Achse,
proportional zu ihrer Entfernung von dieser Achse. Im Maschinenbau tritt dieses
Phänomen z.B. bei der Schubspannung in einem Querschnitt auf: Ein ursprünglich
rechteckiger Querschnitt verformt sich unter Schubbelastung zu einem Parallelogramm.
...
Wert $k=1$ in $x$-Richtung verschoben wird. Ein allgemeiner Punkt ergibt:
...
ursprünglichen Quadrats ist – eine bekannte Eigenschaft der Scherung.
...
| --- | --- | --- |
| Gleichm. Streckung um $s$ | $\begin{pmatrix} s & 0 \\ 0 & s \end{pmatrix}$ | $s^2$ |
| Spiegelung an $y = x$ | $\begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}$ | $-1$ |
| Spiegelung an $x$-Achse | $\begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}$ | $-1$ |
| Scherung in $x$-Richtung | $\begin{pmatrix} 1 & k \\ 0 & 1 \end{pmatrix}$ | $1$ |
| Projektion auf $x$-Achse | $\begin{pmatrix} 1 & 0 \\ 0 & 0 \end{pmatrix}$ | $0$ |
...
Transformationen im $\mathbb{R}^2$ beschreiben: Strecken, Spiegeln und Scheren
sind allesamt lineare Abbildungen der Form $\vec{w} = \mathbf{A} \cdot \vec{v}$.
Die Determinante liefert dabei eine wichtige Zusatzinformation über die Änderung
des Flächeninhalts und die Orientierung. Im nächsten Kapitel erweitern wir diese
Ideen auf den dreidimensionalen Raum und lernen weitere maschinenbaulich relevante
Transformationen kennen, wie die Projektion auf eine Ebene und die Vorbereitung
auf Drehmatrizen.
