...
$\mathbf{X} \cdot \mathbf{A} = \mathbf{C}$ mit $\mathbf{C} = \begin{pmatrix} 0 &
2 \\ 2 & 0 \end{pmatrix}$. Jetzt steht $\mathbf{A}$ rechts von der Unbekannten,
also multiplizieren wir von rechts mit $\mathbf{A}^{-1}$ und erhalten
...
Die Probe $\mathbf{X} \cdot \mathbf{A} = \begin{pmatrix} 1 & 3 \\ 1 & 1 \end{pmatrix} \cdot \begin{pmatrix} 3 & -1 \\ -1 & 1 \end{pmatrix} =
\begin{pmatrix} 0 & 2 \\ 2 & 0 \end{pmatrix}$ bestätigt das Ergebnis. Hätten wir
versehentlich von links multipliziert, hätten wir $\mathbf{A}^{-1} \cdot
\mathbf{C} = \begin{pmatrix} 1 & 1 \\ 3 & 1 \end{pmatrix}$ erhalten, und die
Probe wäre gescheitert.
